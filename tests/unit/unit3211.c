                      ...
                      unsigned int *s, size_t slen)
{
  ...
  fail_unless(!Curl_bset_uint_init(&bset, capacity), "bset init() failed");
  c = Curl_bset_uint_capacity(&bset);
  fail_unless(c == (((capacity + 63) / 64) * 64), "wrong capacity");

  Curl_bset_uint_clear(&bset);
  c = Curl_bset_uint_count(&bset);
  fail_unless(c == 0, "set count is not 0");

  for(i = 0; i < slen; ++i) { /* add all */
    fail_unless(Curl_bset_uint_add(&bset, s[i]), "failed to add");
    for(j = i + 1; j < slen; ++j)
      fail_unless(!Curl_bset_uint_contains(&bset, s[j]), "unexpectedly found");
  }

  for(i = 0; i < slen; ++i) { /* all present */
    fail_unless(Curl_bset_uint_contains(&bset, s[i]), "failed presence check");
  }

  /* iterator over all numbers */
  fail_unless(Curl_bset_uint_first(&bset, &n), "first failed");
  fail_unless(n == s[0], "first not correct number");
  for(i = 1; i < slen; ++i) {
    fail_unless(Curl_bset_uint_next(&bset, n, &n), "next failed");
    if(n != s[i]) {
  ...
  c = Curl_bset_uint_capacity(&bset);
  fail_unless(!Curl_bset_uint_add(&bset, c), "add out of range worked");
  /* The count it correct */
  c = Curl_bset_uint_count(&bset);
  fail_unless(c == slen, "set count is wrong");

  for(i = 0; i < slen; i += 2) { /* remove every 2nd */
    Curl_bset_uint_remove(&bset, s[i]);
    fail_unless(!Curl_bset_uint_contains(&bset, s[i]), "unexpectedly found");
  }
  for(i = 1; i < slen; i += 2) { /* others still there */
    fail_unless(Curl_bset_uint_contains(&bset, s[i]), "unexpectedly gone");
  }
  /* The count is half */
  c = Curl_bset_uint_count(&bset);
  ...
  for(i = 0; i < slen; i++) { /* none present any longer */
    fail_unless(!Curl_bset_uint_contains(&bset, s[i]), "unexpectedly there");
  }
  ...
  fail_unless(!Curl_bset_uint_resize(&bset, capacity * 2),
              "resize double failed");
  for(i = 0; i < slen; i++) { /* all still present after resize */
    fail_unless(Curl_bset_uint_contains(&bset, s[i]), "unexpectedly lost");
  }

  fail_unless(!Curl_bset_uint_resize(&bset, capacity), "resize back failed");
  for(i = 0; i < slen; i++)  /* all still present after resize back */
    fail_unless(Curl_bset_uint_contains(&bset, s[i]), "unexpectedly lost");

  fail_unless(!Curl_bset_uint_resize(&bset, capacity/2), "resize half failed");
  ...
  fail_unless(n == Curl_bset_uint_count(&bset), "set count(halfed) wrong");
  for(i = 0; i < n; i++)  /* still present after resize half */
    fail_unless(Curl_bset_uint_contains(&bset, s[i]), "unexpectedly lost");

  Curl_bset_uint_destroy(&bset);
}
