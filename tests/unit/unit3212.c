...
{
  return Curl_uint_tbl_init(&tbl, TBL_SIZE, NULL);
}
...
for(i = 1; i < TBL_SIZE; i += 2) {
  ...
  fail_unless(Curl_uint_tbl_get(&tbl, i) == &dummy, "does not contain dummy");
}
/* get the first key */
...
fail_unless(!Curl_uint_tbl_resize(&tbl, TBL_SIZE * 2), "error doubling size");
